...
# 1) Configure the data interface (eth1) from ip-mapping.txt
# ──────────────────────────────────────────────────────────────────────────────
log "Configuring data interface $IFACE_DATA"
for _ in {1..70}; do ip link show "$IFACE_DATA" &>/dev/null && break; sleep 0.5; done
...
# ──────────────────────────────────────────────────────────────────────────────
# 2) k3s single-node server with ISOLATED flannel
# - API is reachable on the container's "API iface" (default eth0)
# - Flannel now uses cni0 and vxlan to avoid interfering with eth1 routes
# - TLS cert includes the API IP so other containers can use this kubeconfig
# - Traefik/metrics-server/servicelb disabled to keep it light
# ──────────────────────────────────────────────────────────────────────────────
...
if ! pgrep -f "k3s server" >/dev/null 2>&1; then
  ...
  log "Starting k3s server (API_IP=${API_IP}, NODE_IP=${NODE_IP_DATA}, DATA_IFACE=${IFACE_DATA})"
  ...
  log "k3s started (flannel isolated to cni0/vxlan, eth1 left untouched)"
else
...
if k3s kubectl --kubeconfig "$KCFG" wait --for=condition=Ready node --all --timeout=300s >/dev/null 2>&1; then
  ...
  echo timeout
  log "WARNING: Node not ready within timeout, continuing anyway"
  log "Last 120 lines of $LOG:"
  tail -n 120 "$LOG" || true
fi

# ──────────────────────────────────────────────────────────────────────────────
# CRITICAL: Re-ensure data plane route after k3s initialization
# This protects against any route changes during k3s startup
# ──────────────────────────────────────────────────────────────────────────────
log "Re-ensuring data plane route integrity"
sleep 3  # Give k3s a moment to settle

if [[ -n "$GATEWAY" ]]; then
  # Verify route still exists, restore if missing
  if ! ip route show | grep -q "10.0.0.0/8 via $GATEWAY dev $IFACE_DATA"; then
    log "WARNING: Data plane route was modified, restoring..."
    ip route replace 10.0.0.0/8 via "$GATEWAY" dev "$IFACE_DATA"
  fi
  log "Data plane route confirmed: 10.0.0.0/8 via $GATEWAY dev $IFACE_DATA"
fi
...
# ──────────────────────────────────────────────────────────────────────────────
# FINAL: One more route verification before declaring success
# ──────────────────────────────────────────────────────────────────────────────
if [[ -n "$GATEWAY" ]]; then
  if ip route show | grep -q "10.0.0.0/8 via $GATEWAY dev $IFACE_DATA"; then
    log "✓ Data plane routing verified and stable"
  else
    log "✗ WARNING: Data plane route missing! Attempting final restoration..."
    ip route replace 10.0.0.0/8 via "$GATEWAY" dev "$IFACE_DATA" || log "ERROR: Failed to restore route!"
  fi
fi

log "Initialization complete on $HOSTNAME"